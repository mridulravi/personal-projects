...
void set_defaults(void) {
	...
	if(!sim_disk_exists)
	{
		printf("Sim_disk does not exist, creating it\n");
...
// now create a table for block vs uid to which the block is assigned
	...
	if(!sim_disk_exists)
	{
		...
		num_blocks = num_heads * num_sectors * num_tracks;
...
/*		buf = itoa(root_dir_block_no);*/
		sprintf(buf, "%05d",root_dir_block_no);		// how to make it exact 5 bytes
		...
		sprintf(buf, "%05d", data_section_starting_block_no);
...
/*		free(buf);*/
		...
		buf = (char *) malloc(sizeof(char) * num_blocks +1);
		int i;
		...
		char name_dir[18] = "~//////////////0-1";		// first 5B for name, then 0 for dir type, then -1 for owner id
...
/*		char * no_valid_entries = "00";*/
		// dd-mm-yyyy hh-mm-ss
		char dummy[380];// = {'0'};			// p1 and p2 included in dummy
		for(j=0;j<380;j++)
		{
			dummy[j]= '0';
		}
...
/*		char *p2 = "00000";		// p2 is next in chain*/
		...
		bzero(buf, 513);
		printf("break1.3\n");
		strncat(buf, name_dir, 18);
		strncat(buf, permission, 100);
		printf("Timestamp is %s\n", timestamp);
		strncat(buf, timestamp, 14);
...
/*		strcat(buf, p2);*/
	...
	else
	{
		...
		num_blocks = atoi(buf2);
	
		char * buf = (char *) malloc(sizeof(char)*num_blocks + 1);
		bzero(buf, num_blocks+1);
		bytes_read = fread(buf, 1, num_blocks, fp);
		printf("Disk status Map:\n%s\n", buf);
		
		fclose(fp);
		// DSM will start at offset 15 from beginning
...
void shell() {
    ...
    for (i = 0; i < number_of_processes; i++) 
    {
		...
		if(uid>=100)
		{
			printf("Error: Please enter UID less than 100\n");
			exit(0);
		}
