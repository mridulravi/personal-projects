...
static struct ctx_t *ctx_do_create()
{
	...
	ctx->pid = ke->current_pid++;					//setting pid of the process
	ctx->instr_slice = instr_slice;					// we are setting the instr_slice of the process here
...
// this function creates the context of the process
...
// for cloning the context of a process
...
void ctx_execute_inst(struct ctx_t *ctx)
{
	...
	if (ctx_get_status(ctx, ctx_specmode))
		ctx->mem->safe = 0;
	buf = mem_get_buffer(ctx->mem, ctx->regs->eip, 20, mem_access_exec);			// returns pointer to buffer in the simulated memory
	// returns NULL if either page is not found or offset + size exceeds MAXPAGESIZE
	
	if (!buf) {				// if NULL was returned
		buf = &fixed;
		mem_access(ctx->mem, ctx->regs->eip, 20, buf, mem_access_exec);			// access from the user address space
	}
	ctx->mem->safe = mem_safe_mode;

	// basically buf now has the instruction to execute
...
static void ctx_update_status(struct ctx_t *ctx, enum ctx_status_enum status)
{
	...
	if (!(ctx->status & ctx_suspended) &&
		...
		ctx->status |= ctx_running;					// so for our case, it will enter into this and will enable it's running
	else
		ctx->status &= ~ctx_running;
	
	/* Insert context into the corresponding lists. */
	if (ctx->status & ctx_running)
		ke_list_insert_head(ke_list_running, ctx);			//in our case, it will insert it into the head of the ke_list_running
	if (ctx->status & ctx_zombie)
