...
// this function prints the information of the block
int print_info(int block_no, FILE * fp)
{

	char * buf = (char *) malloc(sizeof(char) * 132);

	// print only if user has non-zero permissions
	int uid = isa_ctx->uid;
	if(uid > 99)
	{
		perror("Right now we are allowing uid's till 99\n");
		return -1;
	}
	
	fseek(fp, block_no * 512, SEEK_SET);
	int bytes_read = fread(buf, 1, 132, fp);
	if(bytes_read!=132)
	{
	...
	if(buf[18+uid] != '0')			// if non zero permission, then list it
	{
		...
		// if file then print size	(right now, not printing it, because not maintaining this in metadata now)
...
/*		printf("\n");*/
		...
		// printing correctly, don't know why		
		printf(" %c%c:%c%c:%c%c%c%c %c%c:%c%c:%c%c \n", buf[118], buf[119], buf[120], buf[121], buf[122], buf[123], buf[124], buf[125], buf[126], buf[127], buf[128], buf[129], buf[130], buf[131] );	
...
int ls(int block_no, FILE * fp)
{
	// check whether this one is file or dir, if file then just print and return, if dir then go through the entry list and then print	
/*	printf("In ls: block no passed is %d\n", block_no);*/
	int bytes_read;
	...
	if(buf[18+uid]=='0')
	{
		perror("User does not have sufficient permissions\n");
		return -1;
	}

	// if it is a file, then use print_info function for printing and then return
	if(buf[16] == '1')
	{
		return print_info(block_no, fp);
	}	
	
	// read the entries and print their information
	
	int block_no_of_entry;
	int offset = 132;
	while(offset < 489)
	{
		if(buf[offset] == '1')
		{
			...
			for(i = 0;i<5;i++)
			{
				num[i] = buf[offset +16+ i];
			}
			num[5] = '\0';
			block_no_of_entry = atoi(num);
			print_info(block_no_of_entry, fp);
		}
		offset += 21;
	}
	...
	child[5] = '\0';
	int pointer = atoi(child);
	
/*	printf("In ls function, pointer is %d\n", pointer);*/
	...
	while(1)
	{
		...
		while(offset < 483)
		{
			if(buf[offset ] == '1')
			{
				...
				num[5] = '\0';
				block_no_of_entry = atoi(num);
/*				printf("///////block_no_of_entry is %d\n", block_no_of_entry);*/
				
				print_info(block_no_of_entry, fp);
/*				if(!print_info(block_no_of_entry, fp))*/
/*				{*/
/*					printf("Error in printing for file starting with block %d\n", block_no_of_entry);*/
/*				}*/
			}
			offset += 21;
		}
...
int rename_in_metadata_block(int block_no, char * name_to_be_printed, char * timestamp, FILE * fp)
{
	// rename the name field here
	// modify the timestamp also

	int offset = block_no * 512;
	fseek(fp, offset, SEEK_SET);

	int temp_count = 0;
	int bytes_written = fwrite(name_to_be_printed, 1, 15, fp);
	
	while(bytes_written!=15)
	{
		fseek(fp, offset, SEEK_SET);
		bytes_written = fwrite(name_to_be_printed, 1, 15, fp);
		temp_count++;
		if(temp_count>=15)
		{
			perror("Error in writing directory name in metadata, inconsistency may occur");
			return -1;
		}
	}
	
	// now modify the timestamp
	offset+=118;
	fseek(fp, offset, SEEK_SET);
	bytes_written = fwrite(timestamp,1, 14, fp);
	
	if(bytes_written!=14)
	{
		perror("Error in modifying timestamp of the file");
		return -1;
	}
...
int delete_dir_entry(int block_no_of_parent, char *name, FILE * fp)
{
	return 0;
}
...
int handle_guest_syscalls() {
    ...
    switch (syscode) {
		...
		/* 404*/
		case syscall_code_pwd:
		{
			// write the syscall for pwd
			
			// find the entry in curr_dir_array which corresponds to this process
			int i =0;
			for(;i<num_processes;i++)
			{
				if(curr_dir_array[i].process_id == isa_ctx->pid)
				{
					printf("\nCurrent directory is %s\n",curr_dir_array[i].pathname );
					break;
				}
			}
			break;
		...
		// In all the syscalls wherever i have used exit(1), use return -1 instead*/*/
		/*405*/
		case syscall_code_ls:
		{
			// must have 1 parameter, if want to print current directory, then give . else give the path, relative or absolute
		
			// write the syscall for ls
			printf("\n\nIn ls sycall\n");
			// first calculate the absolute address
			
			// ebx,ecx, edx, esi, edi and ebp
			uint32_t rel_addr = isa_regs->ebx;
			char rel_path[MAX_PATH_SIZE];
			mem_read(isa_mem, rel_addr, MAX_PATH_SIZE, rel_path);
			
			// call this function only if anything other than "." is given
			char * absolute_path = convert_to_absolute_address(rel_path);
			char * name = get_last_part_of_address(absolute_path);
...
/*			printf("Name is %s\n", name);*/
			...
			// then start traversing from the root block's metadata untill you reach that directory(check permissions aong the way)
			int b_no = 0;
			if(strcmp(absolute_path, "~"))
			{
				b_no = check_permissions_on_path(absolute_path);			// returns -1 if not sufficient prmissions or if invalid address
/*				printf("b_no in ls is %d\n", b_no);*/
			}
			...
			if(!strcmp(absolute_path,"~"))
			{
				retval = ls(root_dir_block_no, fp);
				fclose(fp);
				return retval;		
			}
			...
			if(b_no == -1)
			{
				perror("Either incorrect path or insufficient permissions\n");
				return -1;
			}
			
			// Now b_no is the block no of the metadata block which contains our last directory
			// Now find the directory/file entry from the entry list and then go to the metadata block of that
			...
			char * buf = (char *)malloc(sizeof(char) * 381);
			int bytes_read;
			int count = 0;
...
/*						printf("Entry found\n");*/
			...
			if(entry_found)
			{
				...
				// go to that block and do the required thing
				// create a function called ls which will take the block no as parameter and will do the rest of the thing
				retval = ls(block_no_of_required_entry, fp);
				fclose(fp);
				return retval;
			}
		...
		/*
		int syscall(406, "..."); 
		assume addr doesn't have just . as addr'
		*/
		case syscall_code_makedir:
		{
		
			printf("\n\nIn makedir syscall\n");
			
			int addr_pointer = isa_regs->ebx;
			char filepath[MAX_PATH_SIZE];
			int length;
			length = mem_read_string(isa_mem, addr_pointer, MAX_PATH_SIZE, filepath);
			if(length > MAX_PATH_SIZE)
			{
				printf("Directory path entered is too long\n");
				return -1;
			}
			char * full_path = convert_to_absolute_address(filepath);
			if(!full_path)
			{
...
/*			printf("////////////The full path is %s\n", full_path);*/
			int block_num = check_permissions_on_path(full_path);
/*			printf("The block_num is %d\n", block_num);*/
			if(block_num == -1)
			{
				printf("You do not have the required permissions or the path is incorrect\n");
				return -1;
			}
			//  get block no of first metadata block of parent dir, used when chaining occurs
			int first_block_of_parent = block_num;
			
			// get dir name which we have to make
/*			printf("////////////Befor calling get_last_part_of_address(), full_path : %s\n",full_path);*/
			char * dir_name = get_last_part_of_address(full_path);
/*			printf("////////////After calling get_last_part_of_address(), dir_name : %s\n",dir_name);*/
			
			// now check the name is a valid dir name, and also if name exists in dir. return -1 or 1
			int valid_name = is_valid_dir_name(dir_name, block_num);
/*			printf("////////////After is_valid\n");*/
			if(valid_name == -1)
			{
				printf("The directory is invalid or already in use in current dir\n");
				return -1;
			}
			
			// get free block for new dir metadata block
/*			int free_block = get_free_block(block_num);*/
			
			
			// now go to parent dir, find a free dir entry space, fill it. update dsm.
			// if all dir entries filled , check if chaining already done for this dir, if not do it.
			
			FILE * fp;
			int offset = block_num*512 + 132;
		    fp = fopen("Sim_disk", "r+");
		    if(!fp)
		    {
			perror("Error in opening file Sim_disk\n");
			...
			// check if free dir entry	
			int itr_num = 0;
			int free_dir_entry_byte;
			int found_free_entry = 0;
		
/*			printf("//////////////////Break 111.0\n");*/
/*			printf("*/
/*			printf("Block num before checking free entry in makedir is %d\n", block_num);*/
			for(;itr_num<17;itr_num++)
			{
				if(buf[itr_num*21] == '0')
				{
					free_dir_entry_byte = offset + itr_num*21;
/*					printf("We have found free entry in makedir syscall\n");*/
					found_free_entry = 1;
					break;
				}
			}
...
/*				printf("////////// Pointer is %d\n", ptr);*/
				...
				while(1)
				{
					if(found_free_entry == 1)
					{
						break;
					}
					// not found free entry. check if chaining occurs, if not create new block for dir metadata and set ptr and update dsm
					// check if chaining occurs
				
					if(ptr == 0)
					{
						// allocate new block, chain, update dsm and use first dir entry.
						int new_block = get_free_block(block_num);
						if(new_block == -1)
						{
							perror("No space is available\n");
							fclose(fp);
							return -1;
						}
						
						free(buf);
						buf = (char *) malloc(sizeof(char) * 513);
						bzero(buf, 513);
/*						int dum = 0;			*/
						sprintf(buf, "%0512d", 0);
						offset = new_block * 512;
						fseek(fp, offset, SEEK_SET);
						int bytes_written = fwrite(buf, 1, 512, fp);					
						if(bytes_written!=512)
						{
							perror("Error in writing to Sim_disk");
							return -1;
						}	
						///////////////////////////////////////////
						// chain
						...
						// next ptr
						char b[6];
						sprintf(b, "%05d", new_block);
						offset = block_num*512 + 507;
						fseek(fp, offset, SEEK_SET);
						bytes_written = fwrite(b, 1, 5, fp );
						if ( bytes_written != 5)
						{
							perror("Error in writing to sim_disk");
							fclose(fp);
						...
						// prev ptr of new dir metadata block (of parent dir)
						char b1[6];
						sprintf(b1, "%05d", block_num);
						offset = new_block*512 + 502; // prev ptr
						fseek(fp, offset, SEEK_SET);
						bytes_written = fwrite(b1, 1, 5, fp );
						if ( bytes_written != 5)
						{
							perror("Error in writing to sim_disk");
							fclose(fp);
						...
						// update dsm
						int ret = update_dsm(new_block,fp, 1);
						if(ret == -1)
						{
							perror("Error in writing to sim_disk");
							fclose(fp);
						...
						// use new block for new dir entry
						found_free_entry = 1;
						free_dir_entry_byte = new_block*512;
						block_num = new_block; // use block_num after the while loop		
					}
				
					else
					{
/*						printf("//////////////////Break 111.12\n");*/
						// num_block is now ptr
						block_num = ptr;
/*						printf("Block num(ptr) is %d\n", block_num);*/
...
/*						printf("//////////////////Break 111.15\n");*/
						if(bytes_read != 512)
							...
							// check free dir entry in block
						itr_num = 0;
/*						printf("//////////////////Break 111.2\n");*/
						for(;itr_num<23;itr_num++)
						{
							if(buf[itr_num*21] == '0')
							{
								free_dir_entry_byte = offset + itr_num*21;
...
/*						printf("//////////////////Break 111.4\n");*/
			...
			// have found free entry.
			// free_dir_entry_byte has byte no where we have to write new dir entry ( yet to allocate block for new dir)	
			// allocate new dir block, set its fields
			int bytes_written;
			int new_block_num = get_free_block(block_num);
/*			printf("New block num is %d & %d\n", new_block_num, block_num);*/
			if(new_block_num == -1)
			{
				perror("No space for the new directory in disk");
				fclose(fp);
				return -1;
			}
		
			// update dsm
			int ret = update_dsm(new_block_num, fp, 1);
			if(ret == -1)
			{
				perror("Error in writing to sim_disk");
				fclose(fp);
			...
			// get name of new dir in 15 bytes
			char * temp = dir_name;
			dir_name = (char *) malloc(sizeof(char) * 16);
			bzero(dir_name, 16);
			count = 0;
			while(temp[count])
			{
				dir_name[count] = temp[count];
				count++;
			}
			if(count<15)
			{
			...
			char* dir_type = "0";
			char owner[3];
			sprintf(owner, "%02d", isa_ctx->uid);
			char permissions[100];// = {'0'};
			int j;
			for(j=0;j<100;j++)
			{
				permissions[j] = '0';
			}
			permissions[isa_ctx->uid] = '6';
			char * curr_time = get_current_time();
/*			char dummy[381];*/
			char * dummy = (char *)malloc(sizeof(char) * 381);
			bzero(dummy, 381);
			sprintf(dummy, "%0380d", 0);		
/*			for(j=0;j<300;j++)*/
/*			{*/
/*				dummy[j] = '0';*/
/*			}*/
...
/*			printf("Break 1...1\n");*/
			...
			free(buf);
/*			printf("Break 1...2\n");*/
			buf = (char *)malloc(sizeof(char) * 520);
			bzero(buf, 520);
			strncat(buf, dir_name, 15);
			strncat(buf, dir_type, 1);
			strncat(buf,owner, 2);
			strncat(buf, permissions, 100);
			strncat(buf, curr_time, 14);
			strncat(buf, dummy, 380);
/*			printf("Break 1...3\n");*/
			free(dummy);
/*			printf("New dir metadata block is %s\n", buf);*/
/*			printf("Writing at block %d\n", new_block_num);*/
			offset = new_block_num*512;
			fseek(fp, offset, SEEK_SET);
...
/*			printf("Break 1...4\n");*/
			char* valid = "1"; //valid byte
			char ptr_to_dir[6];
			sprintf(ptr_to_dir, "%05d", new_block_num);	
			bzero(buf, 22);	
			strncat(buf,valid, 1);
			strncat(buf,dir_name, 15);
			strncat(buf,ptr_to_dir, 5);
/*			printf("Break 1...5\n");*/
			offset = free_dir_entry_byte;
			fseek(fp, offset, SEEK_SET);
			
/*			printf("Directory entry which will be inserted is %s\n", buf);*/
/*			printf("writing at offset %d\n", offset);			*/
			
			bytes_written = fwrite(buf, 1, 21, fp );
			if ( bytes_written != 21)
			{
				perror("Error in writing to sim_disk");
				fclose(fp);
			...
			// change timestamp of parent dir
			//char * curr_time = get_current_time();
			offset = first_block_of_parent*512 + 118;
			fseek(fp, offset, SEEK_SET);
			bytes_written = fwrite(curr_time, 1, 14, fp );
			if ( bytes_written != 14)
			{
				perror("Error in writing to sim_disk");
				fclose(fp);
			...
			// close the sim disk file	
			fclose(fp);
			break;
		}		
		////////////////////////////////////////////
		// case 407//
		case syscall_code_cd:
		{
			printf("\n\nIn cd syscall\n");
			...
			char * absolute_path;
			absolute_path = convert_to_absolute_address(rel_path);
			char * last_name = get_last_part_of_address(absolute_path);
		
/*			printf("rel_path is %s\n", rel_path);*/
/*			printf("absolute_path is %s\n", absolute_path);*/
			...
			{	
				...
				for(i = 0; i<num_processes;i++)
				{
					if(curr_dir_array[i].process_id == isa_ctx->pid)
					{
						strcpy(curr_dir_array[i].pathname, absolute_path);
						curr_dir_array[i].block_no = root_dir_block_no;
						retval =0;
						return retval;
					}
				}
			...
			int block_no = check_permissions_on_path(absolute_path);
/*			printf("Block no returned in cd is %d\n", block_no);*/
...
/*			printf("\n\n\nIn Cd, before get_metadata_block_no_of_child\n");*/
			int child_block_no = get_metadata_block_no_of_child(block_no, last_name, fp);
/*			printf("\nIn Cd, after get_metadata_block_no_of_child\n");*/
			
			
			if(child_block_no == -1)
			{
				perror("The given entry does not exist");
				fclose(fp);
				return -1;	
			}
		
			fseek(fp, child_block_no * 512, SEEK_SET);
			char * buf = (char *)malloc(sizeof(char) * 512);			
			int bytes_read = fread(buf, 1, 512, fp);
			...
			if(buf[15] == '1')
		    {
		    	perror("Can't cd to a file");
		    	fclose(fp);
		    	return -1;
		    }
				
			// else we have reached this directory
			// check whether we have write permission
			
/*			printf("Permission of user is %c\n", buf[18+isa_ctx->uid]);*/
			if(buf[18+isa_ctx->uid]!='2' && buf[18+isa_ctx->uid]!='6')
			{
...
/*			printf("\n\n\nIn Cd, making change in FCB\n");*/
			...
			for(i = 0; i<num_processes;i++)
			{
				...
				{
					strcpy(curr_dir_array[i].pathname, absolute_path);
/*					printf("In cd's for loop, %s\n\n", curr_dir_array[i].pathname);*/
					curr_dir_array[i].block_no = child_block_no;
					break;
				}
			}
...
/////////////////////////////////////////////
		...
		*/
		case syscall_code_createfile:
		{
			...
			if(length > MAX_PATH_SIZE)
			{
				printf("Path entered is too long\n");
				return -1;
			}
			char * full_path = convert_to_absolute_address(filepath);
			if(!full_path)
			...
			// traverse the path and check that user has w permissions on all but the last, return block no of metadata of second last part(dir). return -1 if permissions not there, or if path not exist
...
/*			printf("The block_num is %d\n", block_num);*/
			...
			char * file_name = get_last_part_of_address(full_path);
			
			// now check the name is a valid file name, and also if name exists in dir. return -1 or 1
			int valid_name = is_valid_dir_name(file_name, block_num);
			if(valid_name == -1)
			{
				printf("The directory is invalid or already in use in current dir\n");
				return -1;
...
/*			int free_block = get_free_block(block_num);*/
			...
			// have found free entry.
			// free_dir_entry_byte has byte no where we have to write new dir entry ( yet to allocate block for new file, both metadata and data block)	
			
			// allocate new file metadata block, set its fields
			int bytes_written;
			int new_block_num = get_free_block(block_num);
			if(new_block_num == -1)
			{
				perror("No space for the new file in disk");
				fclose(fp);
				return -1;
			}
			...
			// allocate new file data block in data segment
			
			int new_data_block_num = get_free_block(data_section_starting_block_no);
			if(new_data_block_num == -1)
			{
				perror("No space for the new file in disk");
				fclose(fp);
				return -1;
			...
			ret = update_dsm(new_data_block_num, fp, 1);
			if(ret == -1)
			{
				perror("Error in writing to sim_disk");
				fclose(fp);
			...
			// write all 0's in newly allocated data block
			char zeroes[512];
			int zero_index = 0;
			for(;zero_index<512;zero_index++)
			{
				zeroes[zero_index]='0';
			}
			offset = new_data_block_num*512;
			fseek(fp, offset, SEEK_SET);
			bytes_written = fwrite(zeroes, 1, 512, fp);
			if(bytes_written!=512)
			{
				perror("Error in writing to disk");
				exit(1);
			...
			//////////////////////////////////////////////////////////////
			// write into file matadata block
			
			// get name of new file in 15 bytes
			char * temp = file_name;
			...
			// fields which are there only for file and not dir metadata
			char ptr_to_parent_dir[6];
			sprintf(ptr_to_parent_dir, "%05d", first_block_of_parent);
			
			char open_count[2]; //set it to 0 now
			open_count[0] = '0';
			open_count[1] = '0';
			
			char lock[1];
			lock[0] = 'c'; // file is closed
			
			char start_block_num_in_data_partition[6];  // same as last block here
			sprintf(start_block_num_in_data_partition, "%05d", new_data_block_num);
			
			char num_blocks_used_by_file[6];  // on creating, it is using 1 block
			sprintf(num_blocks_used_by_file, "%05d", 1);
			
			char last_byte_in_last_block[4]; //is 0 right now
			sprintf(last_byte_in_last_block, "%03d", 0);
			
			
			char dummy[360];
			bzero(dummy, 360);
			int dummy_index = 0;
			for(;dummy_index<354; dummy_index++)
			{
				dummy[dummy_index] = '0';
			}
...
/*			printf("Break 1...1\n");*/
			...
			free(buf);
/*			printf("Break 1...2\n");*/
			buf = (char *)malloc(sizeof(char) * 540);
			bzero(buf, 540);
			strncat(buf, file_name, 15);
			strncat(buf,dir_type, 1);
			strncat(buf,owner,2);
			strncat(buf, permissions, 100);
			strncat(buf, curr_time, 14);
			strncat(buf, ptr_to_parent_dir, 5);
			strncat(buf, open_count, 2);
			strncat(buf, lock, 1);
			strncat(buf, start_block_num_in_data_partition, 5);
			strncat(buf, num_blocks_used_by_file, 5);
			strncat(buf, start_block_num_in_data_partition, 5);
			strncat(buf, last_byte_in_last_block, 3);
			strncat(buf, dummy, 354);
/*			printf("Break 1...3\n");*/
...
/*			printf("Break 1...5\n");*/
		...
		////////////////////////////////////////////
		// code 409
		case syscall_code_rename_dir:
		{
			printf("\n\nIn rename syscall\n");
			// ebx,ecx, edx, esi, edi and ebp
			uint32_t rel_addr = isa_regs->ebx;
			uint32_t new_name_addr = isa_regs->ecx;
			char rel_path[MAX_PATH_SIZE], new_name[MAX_PATH_SIZE];
			mem_read(isa_mem, rel_addr, MAX_PATH_SIZE, rel_path);
			mem_read(isa_mem, new_name_addr, MAX_PATH_SIZE, new_name);
			
			char * absolute_path = convert_to_absolute_address(rel_path);
			
			int b_no = check_permissions_on_path(absolute_path);
/*			printf("Block no is %d\n", b_no);			*/
						
			if(b_no == -1)
			{
				// check_permissions_on_path would have printed the error message
				return -1;
			}

			// check whether new_name is valid or not
			int name_status = is_valid_dir_name(new_name, b_no);
/*			printf("Name status is %d\n", name_status);*/
			if(name_status == -1)
			{
				// if not valid name, then error will be printed by is_valid_dir_name function name itself
				return -1;
			}	
			
			// change path name in FCB, curr_dir_array 
			// change name in directory entry list of parent and in the metadata block of the directory
...
/*					printf("Entry is %s\n", entry_name);*/
					...
					if(!check)
					{
						...
						num[5] = '\0';
						block_no_of_required_entry = atoi(num);
/*						printf("We have found the entry\n");*/
						entry_found = 1;
						break;
					}
				}
			...
			if(entry_found)
			{
				// first check whether we have write permission on directory
				fseek(fp, block_no_of_required_entry * 512, SEEK_SET);
				free(buf);
				buf = (char *) malloc(sizeof(char) * 512);
				bzero(buf, 512);
				...
				if(perm != '2' && perm != '6')
				{
					perror("User does not have write permission for directory");
					return -1;
				}
				free(buf);
				// we have found the block
				// first change the entry here
				char name_to_be_printed[15];
				int i =0;
				for(i=0;i<15;i++)
				{
					if(new_name[i]!='\0')
					{
						name_to_be_printed[i] = new_name[i];
					}
					else
					{
						while(i<15)
						{
							name_to_be_printed[i] = '/';
							i++;
						}
						break;
					}
				}
				fseek(fp, offset + count + 1, SEEK_SET);				
				// write 15 bytes here
				int temp_count = 0;				
				int bytes_written = fwrite(name_to_be_printed, 1, 15, fp);
				while(bytes_written!=15)
				{
					...
					if(temp_count >=15)
					{
						perror("Error in renaming the directory, inconsistency may occur");
						return -1;
					}
				}
						
				// now modify the timestamp also
				
				char * timestamp = get_current_time();
				fseek(fp, b_no *512 + 118, SEEK_SET);
				bytes_written = fwrite(timestamp, 1, 14, fp);
				
				if(bytes_written !=14)
				{
					perror("Error in writing timestamp in Sim_disk");
					return -1;
				}
				
				// go to that block and rename it
				retval = rename_in_metadata_block(block_no_of_required_entry, name_to_be_printed, timestamp, fp);

				if(!retval)
				{
					printf("Renaming done, Dir %s renamed to %s\n", name, new_name);
					// Modify in curr_dir_array also(path name is not used in OFT and all those places)
					change_path_in_curr_dir_array(name, new_name, absolute_path);
					fclose(fp);
					return 0;
				}
		...
		////////////////////////////////////////////
		case syscall_code_rename_file:
		{
			printf("\n\nIn rename file syscall\n");
			// ebx,ecx, edx, esi, edi and ebp
			uint32_t rel_addr = isa_regs->ebx;
			uint32_t new_name_addr = isa_regs->ecx;
			char rel_path[MAX_PATH_SIZE], new_name[MAX_PATH_SIZE];
			...
			char * absolute_path = convert_to_absolute_address(rel_path);
			char * file_name = get_last_part_of_address(absolute_path);
		
			int block_no_of_parent = check_permissions_on_path(absolute_path);
			if(block_no_of_parent == -1)
			{
				perror("Insufficient permissions\n");
				return -1;
			}
			...
			// check whether new_name is valid or not
			int name_status = is_valid_dir_name(new_name, block_no_of_parent);
/*			printf("Name status is %d\n", name_status);*/
			if(name_status == -1)
			{
				// if not valid name, then error will be printed by is_valid_dir_name function name itself
			...
			int child_block_no = get_metadata_block_no_of_child(block_no_of_parent, file_name, fp);
			...
			// Now if the file is open, then print error message 		
			// so compare the block no in the oft's with it 
			struct oft * temp;
			for(temp = head_oft;temp;temp = temp->next)
			{
				if(temp->block_no_of_file_metadata == child_block_no)
				{
					perror("File is open, cannot rename it");
					fclose(fp);
					return -1;
				}
			...
			// Now check the permissions of the file
			fseek(fp, child_block_no * 512, SEEK_SET);
			char * buf = (char *) malloc(sizeof(char) * 512);
			bzero(buf, 512);
			...
			// check permission, should have atleast write permission
			char perm = buf[18+isa_ctx->uid];
			if(perm!='2' && perm!='6')
			{
				perror("Insufficient permissions");
			...
			// if this is not a file, then return
			if(buf[15] != '1')
			{
				perror("Not a file");
				return -1;
			...
			// since it has reached till this point, therefore everything is fine, now actually perform the rename
			
			int offset = block_no_of_parent * 512 + 132;
			
			fseek(fp, offset, SEEK_SET);
			// now start reading entries, each entry will be 21B
			// can have atmost 17 entries in this page, if it exceeds 17 then use chaining(if the chained block no is non zero)
			// chained pages can have 23 entries in them
			buf = (char *)malloc(sizeof(char) * 380);
			int count = 0;
...
/*						printf("We have found the entry\n");*/
			...
			{
				// first check whether we have write permission on directory
				// we have already checked the permissions
				...
				while(bytes_written!=15)
				{
					...
					if(temp_count >=15)
					{
						perror("Error in renaming the file, inconsistency may occur");
						return -1;
					}
				}
				// now modify the timestamp also
				...
				char * timestamp = get_current_time();
				fseek(fp,block_no_of_parent *512 + 118, SEEK_SET);
				bytes_written = fwrite(timestamp, 1, 14, fp);
				...
				if(!retval)
				{
					printf("Renaming done, file %s renamed to %s\n", name, new_name);
					return 0;
				}
			...
			while(1)
			{
				...
				if(entry_found)
				{
					...
					// first check whether we have write permission on directory
					// we have already checked permissions
		...
		// 415
/*		
		case syscall_code_delete_dir:
		{
			printf("\n\nIn delete_dir syscall\n");
			// ebx,ecx, edx, esi, edi and ebp
			uint32_t rel_addr = isa_regs->ebx;
			char rel_path[MAX_PATH_SIZE], new_name[MAX_PATH_SIZE];
			mem_read(isa_mem, rel_addr, MAX_PATH_SIZE, rel_path);
			...
			int child_block_no = get_metadata_block_no_of_child(block_no_of_parent, name, fp); 
			
			if(child_block_no == -1)
			{
				perror("Directory does not exist, can't delete");
				fclose(fp);
				return -1;
			}
			
			// Now go there and check whether that is empty
			...
			// Now check whether any entry exists or not
			...
			while(count<17*21)
			{
				if(buf[count] == '1')			// valid bit is 1
				{
					perror("Directory not empty, cannot delete");
					fclose(fp);
					return -1;
				}
				count+=21;
			...
			if(pointer == 0)
			{
				// delete the directory, update dsm, and then call delete_dir_entry
				update_dsm(child_block_no, fp, 0);
				retval = delete_dir_entry(block_no_of_parent, name, fp);
				if(retval == -1)
				{
					perror("Error in deleting, inconsistency may occur");
					fclose(fp);
					return retval;
				}
				...
				char * timestamp = get_current_time();
				int offset = block_no_of_parent * 512 + 118;
				fseek(fp, offset, SEEK_SET);
				int bytes_written = fwrite(timestamp, 1, 14, fp);
				if(bytes_written !=14)
				{
					perror("Error in reading from sim_disk");
					fclose(fp);
			...
			while(1)
			{
				...
				int pointer1 = atoi(child_pointer);
				
				if(pointer1 == 0)
				{
					update_dsm(pointer, fp, 0);
					for(i=0;i<5;i++)
					{
						child_pointer[i] = buf[502+i];
					}
					child_pointer[5] ='\0';					
					pointer1 = atoi(child_pointer);
										
					while(pointer1!=child_block_no)
					{
						fseek(fp, pointer1 * 512, SEEK_SET);
						bytes_read = fread(buf, 1, 512, fp);
						int temp_count = 0;
						while(bytes_read!=512)
						{
							...
							if(temp_count>=15)
							{
								perror("Error in reading from Sim_disk, directory not deleted prorperly, inconsistency may occur");
								fclose(fp);
								return -1;
...
///////////////////////////////////////////////////////////////
	...
	//
	//delete_file
	//syscall(416,"<file path>");
	
	case syscall_code_delete_file:
	{
		...
		//parent_block_num is block number of metadata block of parent dir
		int parent_block_num = check_permissions_on_path(full_path);
		...
		int file_metadata_block_num = get_metadata_block_no_of_child(parent_block_num, file_name, fp);
		if(file_metadata_block_num == -1)
		{
			perror("error on calling get_metadata_block_no_of_child function");
			fclose(fp);
			return -1;
		}
		
		// read the file metadata block
		char * buf = (char *)malloc(sizeof(char) * 512);
		int offset = file_metadata_block_num*512;
		fseek(fp, offset, SEEK_SET);
		int bytes_read;
		bytes_read = fread(buf,1,512,fp);
		if(bytes_read != 512)
	    ...
	    if(buf[15] != '1')
	    {
	    	perror("You are trying to delete a directory");
	    	fclose(fp);
			return -1;
	    }
	    
	    // check that user has at least write permissions
	    if( ! (buf[18+isa_ctx->uid]== '2' || buf[18+isa_ctx->uid]== '6' ) )
	    {
	    	perror("You don't have the permission to delete this file");
	    	fclose(fp);
			return -1;
	    }
	    
	    // check open count is 0, same as checking lock field is 'c'.
	    if(buf[139] != 'c')
	    {
	    	perror("You can't delete the file since another process has also opened it.");
	    	fclose(fp);
			return -1;
	    }
	    ...
	    // store start block number of data portion of the file
	    char temp[6];
	    int i = 0;
	    for(;i<5; i++)
	    {
	    ...
	    temp[5] = '\0';
	    int start_block_num_in_data_part = atoi(temp);
	    
	    //////////////////////////////////////////////////////
	    // remove all cache blocks for this file, start_block_num_in_data_part is available
	    i=0;
	    for(; i< cache.cache_size; i++)
	    {
	    	if(cache.cache_data[i].starting_block == start_block_num_in_data_part)
	    	{
	    		cache.cache_data[i].valid = 0;
	    	}
	    }
		
		/////////////////////////////////////////////////////
		
		// remove all data blocks of file
		
		int curr_data_block_num = start_block_num_in_data_part;
		while(1)
		{
			// read block
			offset = curr_data_block_num*512;
			fseek(fp, offset, SEEK_SET);
			free(buf);
			buf = (char *)malloc(sizeof(char) * 512);
			bytes_read = fread(buf,1,512,fp);
			...
			// delete this data block i.e set its dsm entry to free
			update_dsm(curr_data_block_num, fp,0);
			...
			temp[5] = '\0';
			curr_data_block_num = atoi(temp);
			
			if(curr_data_block_num == 0)
			{
				// all data blocks of file have been deleted
				break;
			}		
		...
		//////////////////////////////////////////////////////
		// remove file metadata block by updating dsm
		update_dsm(file_metadata_block_num, fp,0);
					
		////////////////////////////////////////////////////// 
		// delete dir entry of file ..... (call function))
		
		int val_returned = delete_dir_entry(parent_block_num, file_name, fp );
		if(val_returned == -1)
		{
			perror("Error in deleting the directory entry (in its parent dir) of this file");
			fclose(fp);
			return -1;
		}
		...
		char * timestamp = get_current_time();
		offset = parent_block_num * 512 + 118;
		fseek(fp, offset, SEEK_SET);
		int bytes_written = fwrite(timestamp, 1, 14, fp);
		if(bytes_written !=14)
		{
