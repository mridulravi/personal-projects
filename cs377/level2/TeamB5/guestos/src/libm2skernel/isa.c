 ...
 */
 ...
 // here 4th entry is 0, because size of a register cannot be 3 bytes
static uint32_t isa_bit_mask[5] = { 0, 0xff, 0xffff, 0, 0xffffffff};
uint32_t isa_load_reg(x86_register_t reg)
{
	uint32_t mask, *preg;
	mask = isa_bit_mask[isa_reg_info[reg].size];			// chooses the correct entry from isa_bit_mask[] using the size of the register, i.e. if register is of size 1, then choose 0xff, i.e. 8 ones
