...
int get_metadata_block_no_of_child(int block_no, char * child, FILE * fp);		// child is just the name and not path
int get_free_block(int calling_block_num);
int is_valid_dir_name(char* dir_name, int block_num);
int get_number_of_levels(char * path);				// return the gepth of the path_str
// eg:- if the path is ~/abh/abh1/abh2, then it will return 4
