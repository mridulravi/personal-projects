...
///	printf("\n Value of ctxconfig has come as %s\n\n",ctxconfig);
	...
	if (!config_load(config))					// load the config in hashtable struct
	...
	for (ctxnum = 0; ; ctxnum++) {
		...
		// added by me
		curr_dir_array[ctxnum].process_id = ctx->pid;
		curr_dir_array[ctxnum].user_id = ctx->uid;
		curr_dir_array[ctxnum].block_no = root_dir_block_no;
		curr_dir_array[ctxnum].pathname[0] = '~';
		curr_dir_array[ctxnum].pathname[1] = '\0';
