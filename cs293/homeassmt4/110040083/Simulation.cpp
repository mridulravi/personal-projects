   ...
   {	
	...
	{	
		if(SvrIt->busy == 0) //if idle server found
		{
			...
			temp = simTime+tempReq2.getServiceTime();//cout << temp<<"----------"<<endl;
			if (temp >= Maxtemp) Maxtemp = temp;//cout << Maxtemp<<"----------"<<endl;
...
void Simulation::probe_event_handler(Event e) {
   SerOut << ProbeNumber <<"  "<<sys.Buffersize()<<"  "<<serverSpeed<<"  ";
   if (sys.Buffersize() > Jhigh)
   {
	serverSpeed+=S;
