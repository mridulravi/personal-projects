...
Event::Event(int id=0, eventType ty=ARRIVAL, double ti=0.0, int n=0) {
  ID=id; type=ty; time=ti; sn=n;
}
...
void Event::print(ostream & outstream) const {
  outstream << "ID: " << ID << ", type=" << type << ", time=" << time << ", sn=" << sn <<endl;
}
  
double Event::getTime() const { return time;}
int Event::getType() const {return type;}
int Event::getServerNo() {return sn;}
