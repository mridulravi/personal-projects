...
void Simulation::run(QueueingSystem q){
  ...
  //waitTimeVector sorted to get 90% of waiting time
  sort (waitTimeVector.begin(), waitTimeVector.end());	

  //speedvector sorted to get 90% of speed			
   ...
   {	
	...
	{	
		if(SvrIt->busy == 0)					//if idle server found
		{
			...
			sumRespTimes += serviceTime[e.getTime()];	// Response time = service time,as waitingTime=0
			waitTimeVector.push_back (0);			// since waiting time is zero,
									// 0 is pushed into waitTimeVector
 ...
 if (sys.Buffersize()!=0)			// if some request is there in the buffer
{
   ...
   while(SvrIt != ServerList.end())
   {
	...
	/* waiting time is pushed into waiting time vector*/
...
/*********
Probe event handler
The job queue size is checked every P seconds and speed
change action may be taken. At a probe, whenever job queue length (waiting requests only) 
is seen above threshold Jhigh, speed is increased by S (upto max). Whenever
job queue length is seen below threshold Jlow, speed is decreased by S (upto min).
...
void Simulation::probe_event_handler(Event e) {
   ...
   SerOut << serverSpeed <<endl;				// printing server.log
   speedvector.push_back(serverSpeed);				// server speed is pushed into serverSpeed vector
   ProbeNumber++;
   //Next probe event needs to be scheduled, i.e. created and added to
   //event list
   
	/*Probe events are pushed into the event list only until the time when no arrivals are left 
       	  and no departures are left.	
	*/
