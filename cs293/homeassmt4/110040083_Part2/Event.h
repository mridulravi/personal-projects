...
namespace EventTypeNames { enum eventType {ARRIVAL,DEPARTURE,PROBE};}
...
class Event {
 ...
 private:
  ...
  int sn;
 public:
  Event() {ID=0; type=ARRIVAL; time=0.0; sn=0;}  //default constructor
					  	 //creates an arrival event
					 	 //at time 0
  Event(int id, eventType ty, double ti, int sn);  
  ...
  int getServerNo() ;		                 //accessor method
};
