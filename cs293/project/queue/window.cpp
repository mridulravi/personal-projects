...
const int IdRole = Qt::UserRole;
 ...
 Window::Window()
 {
     ...
     shapeComboBox->addItem(tr("Dequeue"), RenderArea::Dequeue);	// adding items to the comboBox
     shapeComboBox->addItem(tr("Enqueue"), RenderArea::Enqueue);	// adding items to the comboBox
     

     shapeLabel = new QLabel(tr("&Queue:"));	// making shapeLabel point to a QLabel storing "Stack"  (& in front of S means a shortcut "Alt+S")
     ...
     brushChanged();
     fontChanged();
     setWindowTitle(tr("QUEUE"));								// setting the title of the window
 }
