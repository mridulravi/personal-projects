...
(define (queens n)
  ...
  (define (safe fullpos1 fullpos2)
    (not (or (= (car fullpos1) (car fullpos2))
             (= (cdr fullpos1) (cdr fullpos2))
...
(define (height tr)
  (if (and (nulltree? (node-ltree tr)) (nulltree? (node-rtree tr)))
      0
      (+ 1 (max (height (node-ltree tr))
                (height (node-rtree tr))))))

(define (leaves tr)
  (if (and (nulltree? (node-ltree tr)) (nulltree? (node-rtree tr)))
      (list (node-val tr))
      (append (leaves (node-ltree tr))
              (leaves (node-rtree tr)))))
...
(define (diameter tr)
  (if (and (nulltree? (node-ltree tr)) (nulltree? (node-rtree tr)))
      0
      (+ 2 (diameter (node-ltree tr)) (diameter (node-rtree tr)))))
...
(define (join lt rt)
  (cond ((nulltree? rt) lt)
        ((nulltree? lt) rt)
	(else (node (rightmost lt) (readjust lt) rt))))
