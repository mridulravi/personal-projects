...
// will define an overloaded function ""create"" which will take a list of Ast * as an argument, and which will put the value from the ast * in the eval_env for the arguments and initialize other with 0
/*void Symbol_Table::create(Local_Environment & local_global_variables_table, list<Ast*> l, int num)
{
	list<Symbol_Table_Entry *>::iterator i;
	int count = 0;
	list<Ast *>::iterator itr = l.begin();
	...
	for (i = variable_table.begin(); i != variable_table.end(); i++)
	{
		...
		if(count < num)
		{	
			// here get the value from the list and put that value in the symbol table
			if((*itr)->get_data_type() == int_data_type)
			{
				Eval_Result_Value_Int & my_res = *new Eval_Result_Value_Int();
				my_res = (*itr)->evaluate(eval_env, file_buffer);
				my_res.set_variable_status(true);
				local_global_variables_table.put_variable_value(my_res, name);				
			}
			else
			{
