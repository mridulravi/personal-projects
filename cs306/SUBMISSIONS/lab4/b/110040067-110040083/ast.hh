...
{
// If condition can have relational(now arithmetic also) conditions only, this is because the preprocessor takes care of all other cases, i.e. cases where there is an assignment statement as condition( eg:- if(a = 0) ) or a const value as condition (eg:- if(0) )
	Ast * condition;
	Ast * trueSuccessor;
	Ast * falseSuccessor;
...
class Function_call_Ast:public Ast
{
	...
	list <Ast *> par;
public:
	Function_call_Ast(string name, Data_Type data_type);
	Function_call_Ast(string name, list<Ast *> t, Data_Type data_type);
	~Function_call_Ast();
