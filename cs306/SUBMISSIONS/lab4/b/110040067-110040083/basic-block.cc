...
Eval_Result & Basic_Block::evaluate(Local_Environment & eval_env, ostream & file_buffer)
{
	...
	file_buffer << "\n" << BB_SPACE << "Basic Block: " <<setprecision(0)<< id_number << "\n";
	...
	for (i = statement_list.begin(); i != statement_list.end(); i++)
	{
		...
		// here this portion is wrong, we have to change it, because if there is a function call which does not return anything, then it will stop the execution here itself, which should not happen as it is the return status of the function call and is not the return status of the current procedure 
//		if(result->get_result_enum() == void_result && result->get_functional() == false)
//		{
//			return *result;
//		}
