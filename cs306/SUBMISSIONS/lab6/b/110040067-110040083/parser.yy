...
|
	...
	{	
		if(NOT_ONLY_PARSE)
		{
			...
			$$->push_back(new GoTo_Ast($2, get_line_number() ) );
		}
	}
;
// added here in executable_statement_list for if-else and goto_stmt
...
comparison:
	comparison NE inner_comp
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Relational_Ast($1, RELATIONAL_OP::NE, $3, get_line_number());		//5
			$$->check_ast();
		}
...
|
	comparison EQ inner_comp
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Relational_Ast($1, RELATIONAL_OP::EQ, $3, get_line_number());						//0
			$$->check_ast();
		}
	}
|
...
inner_comp:
	...
	{ 
		if(NOT_ONLY_PARSE)
		{
			$$ = new Relational_Ast($1, RELATIONAL_OP::GE, $3, get_line_number());				//1
			$$->check_ast();
		}
	}
|
	inner_comp GT expression
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Relational_Ast($1, RELATIONAL_OP::GT, $3, get_line_number());					//2
			$$->check_ast();
		}
	}
|
	inner_comp LE expression
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Relational_Ast($1, RELATIONAL_OP::LE, $3, get_line_number());				//3
			$$->check_ast();
		}
	}
|
	inner_comp LT expression
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Relational_Ast($1, RELATIONAL_OP::LT, $3, get_line_number() );				//4
			$$->check_ast();
		}
	}
|
...
expression:
	expression '+' sub_expr
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Arithmetic_Ast($1, ARITHMETIC_OP::PLUS, $3, get_line_number() );
			$$->check_ast();
		}
	}
|
	expression '-' sub_expr
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Arithmetic_Ast($1, ARITHMETIC_OP::MINUS, $3, get_line_number() );
			$$->check_ast();
		}
	}
|	
...
sub_expr:
	sub_expr '*' single_term
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Arithmetic_Ast($1, ARITHMETIC_OP::MULT, $3, get_line_number() );
			$$->check_ast();
		}
	}
|	
	sub_expr '/' single_term
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Arithmetic_Ast($1, ARITHMETIC_OP::DIV, $3, get_line_number() );
			$$->check_ast();
		}
	}
|	
...
|
	'-' variable
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Arithmetic_Ast($2, ARITHMETIC_OP::UMINUS, NULL, get_line_number() );
...
|
	'(' data_type ')' single_term
	{
		...
		{
//			$$ = $4;
			if(typeid(*$4) == typeid(Name_Ast) )
			{
				$$ = new Type_Ast($4, get_line_number(), &($4->get_symbol_entry()) );
			}
			else
			{
				Symbol_Table_Entry * temp = new Symbol_Table_Entry(); 
				$$ = new Type_Ast($4, get_line_number(), temp);
			}
			if($2 == 1)
			{
...
|
	'-' '(' data_type ')' single_term
	{
		...
		{
			Ast * temp;
			if(typeid(*$5) == typeid(Name_Ast) )
			{
				temp = new Type_Ast($5, get_line_number(), &($5->get_symbol_entry()) );
			}
			else
			{
				Symbol_Table_Entry * temp1 = new Symbol_Table_Entry(); 
				temp = new Type_Ast($5, get_line_number(), temp1);
			}
			...
			{
				temp->set_data_type(float_data_type);
			}
			else if($3 == 3)
			{
				temp->set_data_type(float_data_type);
			}
			$$= new Arithmetic_Ast(temp, ARITHMETIC_OP::UMINUS, NULL, get_line_number() );
			$$->set_data_type(temp->get_data_type() );
		}
	}
|
	'(' comparison ')'
...
|
	'-' '(' comparison ')'
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Arithmetic_Ast($3, ARITHMETIC_OP::UMINUS, NULL, get_line_number() );
...
if_goto_else_goto : IF '(' comparison ')' goto_stmt ELSE goto_stmt
	{
		if(NOT_ONLY_PARSE)
		{
			$$ = new Conditional_Ast($3, $5, $7, get_line_number() );
		}
	}
;
                             
goto_stmt : GOTO BBNUM ';'
    {
		if(NOT_ONLY_PARSE)
		{
	    	$$ = new successor($2);
