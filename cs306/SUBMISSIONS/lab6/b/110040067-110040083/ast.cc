...
//const type_info & Ast::get_child_type()
//{
//	stringstream msg;
//	msg << "No get_child_type() function for " << typeid(*this).name();
//	CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH, msg.str());
//}
////////////////////////////////////////////////////////////////
...
Code_For_Ast & Assignment_Ast::compile()
{
	...
	*/
	...
	// here check for those cases where lhs and rhs are of non-compatible types, eg:-lhs is float while rhs is int
...
{
//	cout << "In compile_and_optimize_ast of Assignment_Ast\n";

	CHECK_INVARIANT((lhs != NULL), "Lhs cannot be null");
	CHECK_INVARIANT((rhs != NULL), "Rhs cannot be null");

	// error when both lra and icode are used comes due to this line
	// change in optimize_lra function, use typeid to compare whether RHS is of relational type, if so then call using some other LRA_scenario, otherwise call using same but check inside lra_outcome
	// spim_register_table is public, so in compile_and_optimize_ast of Relational, use that table to get right registers 
	
	if( (typeid(*rhs) != typeid(Relational_Ast) ) && (typeid(*rhs) != typeid(Arithmetic_Ast) ) && (typeid(*rhs) != typeid(Type_Ast) ) )
	{
...
//	if( (typeid(*rhs) == typeid(Relational_Ast) ) || (typeid(*rhs) == typeid(Name_Ast) ) )
	if( (typeid(*rhs) == typeid(Relational_Ast) ) || (typeid(*rhs) == typeid(Arithmetic_Ast) ) || (typeid(*rhs) == typeid(Type_Ast) ) )
	{
...
Code_For_Ast & Name_Ast::compile()
{
	...
	// here if type is float, then use get_new_float_register
	...
	if(node_data_type == float_data_type)
	{
		load_stmt = new Move_IC_Stmt(load, opd, register_opd, true);
	}
	else
	{
...
//	cout << "create_store_stmt 1.0\n";
//	cout << store_register->get_name() << endl;	
//	cout << "create_store_stmt 1.0\n";
	Icode_Stmt * store_stmt;
	if(store_register->get_reg_val_type() == float_num)
	{
//		cout << "create_store_stmt 1.1\n";
	 	store_stmt = new Move_IC_Stmt(store, register_opd, opd, true);
	}
	else
	{
//		cout << "create_store_stmt 1.2\n";
		store_stmt = new Move_IC_Stmt(store, register_opd, opd);
	}
	
//	cout << "create_store_stmt 1.3\n";
	
	if (command_options.is_do_lra_selected() == false)
		variable_symbol_entry->free_register(store_register);

//	cout << "create_store_stmt 1.4\n";
	list<Icode_Stmt *> & ic_list = *new list<Icode_Stmt *>;
...
//	cout << "In compile_and_optimize_ast of Name_Ast\n";
...
Code_For_Ast & Number_Ast<DATA_TYPE>::compile()
{
	...
	if(node_data_type == float_data_type)
	{
		load_stmt = new Move_IC_Stmt(imm_load, opd, load_register, true);
	}
	else
	{
...
{
//	cout << "In compile_and_optimize_ast of Number_Ast\n";
...
//	cout << "In compile_and_optimize_ast of Relational_Ast\n";
	
	if(typeid(*lhs) == typeid(Name_Ast) )
	{
//		cout << " In name of lhs of in Relational_Ast1\n";
		lra.optimize_lra(mc_2r, NULL, lhs);
//		cout << " In name of lhs of in Relational_Ast2\n";
	}
	...
	Symbol_Table_Entry temp1 = * new Symbol_Table_Entry(str1, void_data_type, -1);
	temp1.set_symbol_scope(local);
	o1->update_symbol_information( temp1);
	...
	string str2 = "abh 12";
	Symbol_Table_Entry temp2 = * new Symbol_Table_Entry(str2, void_data_type, -1);
	temp2.set_symbol_scope(local);
	o2->update_symbol_information( temp2);
...
//	o2->clear_lra_symbol_list();
	...
	o1->remove_symbol_entry_from_list(temp1);
	o2->remove_symbol_entry_from_list(temp2);
	return * code;
}

//////////////////////////////////////////////////////////////////////////
Arithmetic_Ast::Arithmetic_Ast(Ast* temp_lhs, ARITHMETIC_OP temp_op, Ast* temp_rhs, int line)
{
	lhs = temp_lhs;
	rhs = temp_rhs;
	op = temp_op;
...
Eval_Result & Arithmetic_Ast::evaluate(Local_Environment & eval_env, ostream & file_buffer)
{
	...
	if(i.is_variable_defined() == false)
	{	
		CHECK_INPUT_AND_ABORT(false, "variable should be defined to be used in Arithmetic operation", lineno);
...
//		report_error("variable should be defined to be used in Arithmetic operation", NOLINE);
	...
	if(lhs->get_data_type() == int_data_type)
	{
		...
		switch(op)
		{
			// here change get_value to get_value
			case 0:
			...
			case 3:
				if(j->get_value() == 0)
				{
					CHECK_INPUT_AND_ABORT(false, "Divide by 0", lineno);
//					report_error("Divide by 0", NOLINE);
			...
			case 4:
				...
				break;
			default:
				CHECK_INPUT_AND_ABORT(false, "Only 5 operators defined", lineno);
...
Code_For_Ast & Arithmetic_Ast::compile()
{
	...
	if(node_data_type == float_data_type)
	{
		res = new Register_Addr_Opd(machine_dscr_object.get_new_float_register() );
		check = true;
	}
	else
	{
		res = new Register_Addr_Opd(machine_dscr_object.get_new_register() );
	}
	Icode_Stmt * stmt;
	...
	switch(op)
	{
		case ARITHMETIC_OP::PLUS:
			{
				// suppose the reg r1 and r2 contains the result of lhs and rhs, then pass them as operands
				stmt = new Arithmetic_IC_Stmt(add, op1, op2, res, check );
				ic_list.push_back(stmt);
				break;
			}
		case ARITHMETIC_OP::MULT:
			{
				// suppose the reg r1 and r2 contains the result of lhs and rhs, then pass them as operands
				stmt = new Arithmetic_IC_Stmt(mul, op1, op2, res, check );
				ic_list.push_back(stmt);
				break;
			}
		case ARITHMETIC_OP::MINUS:
			{
				// suppose the reg r1 and r2 contains the result of lhs and rhs, then pass them as operands
				stmt = new Arithmetic_IC_Stmt(sub, op1, op2, res, check );
				ic_list.push_back(stmt);
				break;
			}
		case ARITHMETIC_OP::DIV:
			{
				// suppose the reg r1 and r2 contains the result of lhs and rhs, then pass them as operands
				stmt = new Arithmetic_IC_Stmt(division, op1, op2, res, check );
				ic_list.push_back(stmt);
				break;
			}
		case ARITHMETIC_OP::UMINUS:
			{
				// suppose the reg r1 and r2 contains the result of lhs and rhs, then pass them as operands
				stmt = new Arithmetic_IC_Stmt(uminus, op1, op2, res, check );
				ic_list.push_back(stmt);
				break;
			}
		default:
			{
				CHECK_INPUT_AND_ABORT(false, "Only 5 operators are allowed", lineno);
				break;
			}
	...
	if(ic_list.empty() == false)
	{
		if(check)
		{
			code = new Code_For_Ast(ic_list, machine_dscr_object.get_new_float_register() );
		}
		else
		{
			code = new Code_For_Ast(ic_list, machine_dscr_object.get_new_register() );
...
//		cout << " In name of lhs of in Relational_Ast1\n";
	...
	//		cout << " In name of lhs of in Relational_Ast1\n";
		...
		o2 = stmt2.get_reg();
		string str2 = "abh 22";
		temp2 = * new Symbol_Table_Entry(str2, void_data_type, -1);
		temp2.set_symbol_scope(local);
		o2->update_symbol_information( temp2);
...
Code_For_Ast & Conditional_Ast::compile()
{
	...
	int label1 = res1.get_value();
	int label2 = res2.get_value();
...
{
	Code_For_Ast & stmt1 = condition->compile_and_optimize_ast(lra);
	Register_Descriptor * reg = stmt1.get_reg();
...
//////////////////////////////////////////////////////////////////////
/*
mtc1 is when we want to convert int to float
mfc1 is when we want to convert float to int

create a new Ast class named type_ast, which will be used for conversions only
will have a member Ast * child
so we will call the compile of child and then we will use mtc or mfc instruction only
*/

Type_Ast::Type_Ast(Ast * temp, int line, Symbol_Table_Entry * s)
{
	child = temp;
	lineno = line;
	ast_num_child = unary_arity;
...
Data_Type Type_Ast::get_data_type()
{
	return node_data_type;
}

void Type_Ast::set_data_type(Data_Type data_type)
{
	node_data_type = data_type;
}

Symbol_Table_Entry & Type_Ast::get_symbol_entry()
{
	return *symbol_entry;
}

void Type_Ast::print(ostream & file_buffer)
{
	child->print(file_buffer);
}

Eval_Result & Type_Ast::evaluate(Local_Environment & eval_env, ostream & file_buffer)
{
	Eval_Result & result = child->evaluate(eval_env, file_buffer);
	return result;
}

Code_For_Ast & Type_Ast::compile()
{
//	cout << "break 1.0\n";
	// compile the child and then use mfc or mtc instruction
	Code_For_Ast & stmt1 = child->compile();
...
//	cout << "break 1.2\n";
	// now use mfc or mtc instruction
	Ics_Opd * res;
	Icode_Stmt * stmt;
	if(node_data_type == float_data_type && child->get_data_type() == int_data_type )
	{
		// use mtc instruction
		res = new Register_Addr_Opd(machine_dscr_object.get_new_float_register() );
		stmt = new Convert_IC_Stmt(mtc1, op1, res);
	}
	else if(node_data_type == int_data_type && child->get_data_type() == float_data_type )
	{
		// use mfc instruction
		res = new Register_Addr_Opd(machine_dscr_object.get_new_register() );
		stmt = new Convert_IC_Stmt(mfc1, op1, res);
	}
...
//	cout << "break 1.3\n";
	...
	if(ic_list.empty() == false)
	{
		if(node_data_type == float_data_type && child->get_data_type() == int_data_type )
		{
//			cout << "Type_Ast compile1.0\n";
			code = new Code_For_Ast(ic_list, machine_dscr_object.get_new_float_register() );
		}
		else if(node_data_type == int_data_type && child->get_data_type() == float_data_type)
		{
//			cout << "Type_Ast compile1.1\n";
			code = new Code_For_Ast(ic_list, machine_dscr_object.get_new_register() );
		}
		else
		{
...
Code_For_Ast & Type_Ast::compile_and_optimize_ast(Lra_Outcome & lra)
{

	if( (typeid(*child) != typeid(Relational_Ast) ) && (typeid(*child) != typeid(Arithmetic_Ast)) )
	{
		lra.optimize_lra(mc_2r, NULL, child);
	}

	Code_For_Ast & stmt1 = child->compile_and_optimize_ast(lra);
...
//	cout << "break 1.2\n";
	...
	if(node_data_type == float_data_type && child->get_data_type() == int_data_type )
	{
		// use mtc instruction
		res = new Register_Addr_Opd(machine_dscr_object.get_new_float_register() );
//		res = new Register_Addr_Opd(lra.get_register() );
		stmt = new Convert_IC_Stmt(mtc1, op1, res);
	}
	else if(node_data_type == int_data_type && child->get_data_type() == float_data_type )
	{
...
//const type_info & Type_Ast::get_child_type()
//{
//	return typeid(child);
//}

// have to check in lra_optimize that whether child 
