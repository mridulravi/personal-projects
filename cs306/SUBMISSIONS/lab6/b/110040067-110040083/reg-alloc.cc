...
Register_Val_Type Register_Descriptor::get_reg_val_type()		{ return value_type;}
...
int Register_Descriptor::get_symbol_list_size()
{
...
//	cout << "Removed from " << reg_name << ": " << sym_entry.get_variable_name()<<endl;
}
...
//	}
	...
	i = lra_symbol_list.begin();
//	cout <<"Break 4.0011\n";
	for (; i != lra_symbol_list.end(); i++)
	{
//		cout <<"Break 4.11\n";
		if (**i == sym_entry)
		{
...
//		cout << "added " << reg_name << ": " << sym_entry.get_variable_name()<<endl;
	}
	else
	{
//		cout << "exists " << reg_name << ": " << sym_entry.get_variable_name()<<endl;
	}	
//	cout << "Break 3.1\n";
}
...
void Lra_Outcome::optimize_lra(Lra_Scenario lcase, Ast * destination_memory, Ast * source_memory)
{
	...
	// destination memory is LHS and source memory is RHS
	...
	case mc_2m:
		...
		// here check for Relational also on rhs
		...
		else
		{
				...
				if(typeid(*destination_memory) == typeid(Type_Ast) )
				{
					if(destination_symbol_entry->get_data_type() == void_data_type)
					{
						destination_register = NULL;
					}
					else
		...
		if (typeid(*source_memory) == typeid(Number_Ast<int>))
			...
			source_register = NULL;
		else if ( typeid(*source_memory) == typeid(Relational_Ast) )
			source_register = NULL;
		else if ( typeid(*source_memory) == typeid(Arithmetic_Ast) )
			source_register = NULL;
		else
		{
			source_symbol_entry = &(source_memory->get_symbol_entry());
				if(typeid(*source_memory) == typeid(Type_Ast) )
				{
					if(source_symbol_entry->get_data_type() == void_data_type)
					{
						source_register = NULL;
					}
					else
		...
		{
			if(destination_register->get_symbol_list_size() == 1)
			{
				// if the register is the only one associated with
			...
			{
				// if the destination register contains more than 1 variable
		...
		//	cout << "Freeing register " << destination_register->get_name() << endl;
		}
//		cout << "Updating register " << result_register->get_name() << endl;
	...
	case mc_2r:
		...
		if (source_register != NULL)
		{
//			cout << "In mc_2r, source_register is not NULL\n";
			result_register = source_register;
			is_same_as_source = true;
			load_needed = false;
		}
		...
		{
//			cout << "In mc_2r, source_register is NULL\n";
	...
	case c2r:
//		cout << "In c2r case of optimize_lra\n";
		CHECK_INVARIANT(source_memory, "Source ast pointer cannot be NULL for m2m scenario in lra");
		source_register = NULL;
		destination_register = NULL;
		if(source_memory->get_data_type() == float_data_type)
		{
...
//		result_register = machine_dscr_object.get_new_register();
		is_a_new_register = true;
//		cout << " exiting from c2r in switch statement\n";
//		return;
		break;
...
void Machine_Description::initialize_instruction_table()
{
	...
	spim_instruction_table[division] = new Instruction_Descriptor(division, "div", "div", "", i_r_o1_op_o2, a_op_r_o1_o2);
...
Register_Descriptor * Machine_Description::get_new_register()
{
	...
	for (i = spim_register_table.begin(); i != spim_register_table.end(); i++)
	{
		...
		if ( (reg_desc->get_reg_val_type() != float_num) && (reg_desc->is_free() ) )
			return reg_desc;
	}
