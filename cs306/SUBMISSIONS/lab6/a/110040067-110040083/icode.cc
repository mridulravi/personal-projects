...
Move_IC_Stmt::Move_IC_Stmt(Tgt_Op op, Ics_Opd * o1, Ics_Opd * res)
{
	...
	opd1 = o1;   
	result = res; 
	is_floating_operation = false;
}

Move_IC_Stmt::Move_IC_Stmt(Tgt_Op op, Ics_Opd * o1, Ics_Opd * res, bool temp)
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
void Move_IC_Stmt::print_icode(ostream & file_buffer)
{
	...
	// this portion is giving error
//	Register_Descriptor *temp = result->get_reg();
//	Register_Val_Type check_type = temp->get_reg_val_type();
...
void Move_IC_Stmt::print_assembly(ostream & file_buffer)
{
	...
	if(is_floating_operation)
	{
		op_name = op_desc.get_ic_symbol();
	}
	else
	{
		op_name = op_desc.get_mnemonic();
	}
...
Relational_IC_Stmt::Relational_IC_Stmt(Tgt_Op op, Ics_Opd * o1, Ics_Opd * o2, Ics_Opd * res)
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
void Relational_IC_Stmt::set_result(Ics_Opd * io)
{
	result = io;
}
...
{
	CHECK_INVARIANT (opd1, "Opd1 cannot be NULL for a RelationalIC Stmt");
	CHECK_INVARIANT (opd2, "Opd2 cannot be NULL for a RelationalIC Stmt");
	CHECK_INVARIANT (result, "Result cannot be NULL for a Relational IC Stmt");
...
Control_Flow_IC_Stmt::Control_Flow_IC_Stmt(Tgt_Op op, Ics_Opd * o1, int a, int b)		// in case of if-else
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
Control_Flow_IC_Stmt::Control_Flow_IC_Stmt(Tgt_Op op, int a)					// in case of goto
{
	opd1 = NULL;
	op_desc = *(machine_dscr_object.spim_instruction_table[op]);
	l1 = a;
...
void Control_Flow_IC_Stmt::print_icode(ostream & file_buffer)
{
	...
	switch(ic_format)
	{
		case i_r_o1_op_o2:
		{
			CHECK_INVARIANT(opd1, "opd1 cannot be NULL for If_Else")

			Instruction_Descriptor op_desc1 =  *(machine_dscr_object.spim_instruction_table[jump]);	
			string op_name2 = op_desc1.get_name();
			
			file_buffer << " " << op_name << ":\t";
			opd1->print_ics_opd(file_buffer);
			file_buffer << " , zero :";
			file_buffer << " " << op_name2 << " label" << l1 << "\n";
			file_buffer << " " << op_name2 << " label" << l2 << "\n"; 
			break;
		}
		case i_op_o1:
		{
			file_buffer << " " << op_name << " label" << l1 << "\n"; 
			break;
		}
		default:
			CHECK_INVARIANT(false, "Wrong ic_format for Control_Flow_IC_Stmt");
	}
}
...
void Control_Flow_IC_Stmt::print_assembly(ostream & file_buffer)
{
	...
	switch(assem_format)
	{
		case a_op_o1_o2_r:
			{
				...
				Instruction_Descriptor op_desc1 = *(machine_dscr_object.spim_instruction_table[jump]);
				string op_name2 = op_desc1.get_mnemonic();
				
				file_buffer << "\t" << op_name << " ";
				opd1->print_asm_opd(file_buffer);
				file_buffer << ", $zero, label" << l1 << "\n";
				file_buffer << "\t" << op_name2 << " label" << l2  << "\n";
				break;
			}	
		case a_op_o1:
		...
		default:
			CHECK_INVARIANT(false, "wrong assem_format for Control_Flow_IC_Stmt");
	}
}

/***********************************Class Arithmetic_IC_Stmt*********************/
Arithmetic_IC_Stmt::Arithmetic_IC_Stmt(Tgt_Op op, Ics_Opd *o1, Ics_Opd *o2, Ics_Opd * res)
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
Arithmetic_IC_Stmt::Arithmetic_IC_Stmt(Tgt_Op op, Ics_Opd *o1, Ics_Opd *o2, Ics_Opd * res, bool temp)
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
Ics_Opd * Arithmetic_IC_Stmt::get_opd1()
{
	return opd1;
}

void Arithmetic_IC_Stmt::set_opd1(Ics_Opd * o1)
{
	opd1 = o1;
}

Ics_Opd * Arithmetic_IC_Stmt::get_opd2()
{
	return opd2;
}

void Arithmetic_IC_Stmt::set_opd2(Ics_Opd * o2)
{
	opd2 = o2;
}

Ics_Opd * Arithmetic_IC_Stmt::get_result()
{
	return result;
}

void Arithmetic_IC_Stmt::set_result(Ics_Opd * res)
{
	result = res;
}

void Arithmetic_IC_Stmt::print_icode(ostream & file_buffer)
{
	string operation_name = op_desc.get_name();

	Icode_Format ic_format = op_desc.get_ic_format();

	CHECK_INVARIANT (opd1, "Opd1 cannot be NULL for a Arithmetic_IC_Stmt");
	if(ic_format!= i_r_op_o1)
	{
		CHECK_INVARIANT (opd2, "Opd2 cannot be NULL for a Arithmetic_IC_Stmt");
	}
	CHECK_INVARIANT (result, "Result cannot be NULL for a Arithmetic_IC_Stmt");
...
void Arithmetic_IC_Stmt::print_assembly(ostream & file_buffer)
{
...
/******************************* Class Convert_IC_Stmt ****************************/
Convert_IC_Stmt::Convert_IC_Stmt(Tgt_Op op, Ics_Opd *o1, Ics_Opd *res)
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
Convert_IC_Stmt & Convert_IC_Stmt::operator=(const Convert_IC_Stmt & rhs)
{
	op_desc = rhs.op_desc;
	opd1 = rhs.opd1;
	result = rhs.result;
...
Ics_Opd * Convert_IC_Stmt::get_opd1()
{
	return opd1;
}

void Convert_IC_Stmt::set_opd1(Ics_Opd * o1)
{
	opd1 = o1;
}

Ics_Opd * Convert_IC_Stmt::get_result()
{
	return result;
}

void Convert_IC_Stmt::set_result(Ics_Opd * res)
{
	result = res;
}

void Convert_IC_Stmt::print_icode(ostream & file_buffer)
{
	CHECK_INVARIANT (opd1, "Opd1 cannot be NULL for a Convert_IC_Stmt");
	CHECK_INVARIANT (result, "Result cannot be NULL for a Convert_IC_Stmt");
...
void Convert_IC_Stmt::print_assembly(ostream & file_buffer)
{
	CHECK_INVARIANT (opd1, "Opd1 cannot be NULL for a Convert_IC_Stmt");
	CHECK_INVARIANT (result, "Result cannot be NULL for a Convert_IC_Stmt");
	string op_name = op_desc.get_mnemonic();
