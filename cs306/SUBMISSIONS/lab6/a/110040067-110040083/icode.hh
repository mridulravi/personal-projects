...
{ 
	Ics_Opd * opd1;   
	Ics_Opd * result;
	bool is_floating_operation; 

public:
	Move_IC_Stmt(Tgt_Op inst_op, Ics_Opd * opd1, Ics_Opd * result); 
	Move_IC_Stmt(Tgt_Op inst_op, Ics_Opd * opd1, Ics_Opd * result, bool temp); 
	~Move_IC_Stmt() {} 
...
public:
	Relational_IC_Stmt(Tgt_Op inst_op, Ics_Opd * o1, Ics_Opd * o2, Ics_Opd * res);
	~Relational_IC_Stmt() {}
...
class Control_Flow_IC_Stmt: public Icode_Stmt
{
	...
	int l1, l2;
public:
	Control_Flow_IC_Stmt(Tgt_Op op, Ics_Opd * o1, int a, int b);			// in case od if-else
	Control_Flow_IC_Stmt(Tgt_Op op, int a);								// in case of goto only
...
class Arithmetic_IC_Stmt: public Icode_Stmt
{
	Ics_Opd * opd1;
	Ics_Opd * opd2;
	Ics_Opd * result;
	bool is_floating_operation;
public:
	Arithmetic_IC_Stmt(Tgt_Op inst_op, Ics_Opd *o1, Ics_Opd *o2, Ics_Opd *res);
	Arithmetic_IC_Stmt(Tgt_Op inst_op, Ics_Opd *o1, Ics_Opd *o2, Ics_Opd *res, bool temp);
	~Arithmetic_IC_Stmt() {}

	Arithmetic_IC_Stmt & operator=(const Arithmetic_IC_Stmt & rhs);

	Ics_Opd * get_opd1();
	void set_opd1(Ics_Opd * o1);
	
	Ics_Opd * get_opd2();
	void set_opd2(Ics_Opd *o2);
	
	Ics_Opd * get_result();
	void set_result(Ics_Opd * res);

	void print_icode(ostream & file_buffer);
	void print_assembly(ostream & file_buffer);
};

class Convert_IC_Stmt: public Icode_Stmt
{
	Ics_Opd * opd1;
	Ics_Opd * result;
public:
	Convert_IC_Stmt(Tgt_Op inst_op, Ics_Opd *o1, Ics_Opd *res);
	~Convert_IC_Stmt() {}
	
	Convert_IC_Stmt & operator=(const Convert_IC_Stmt & rhs);
...
// have to add classes for other statements as well
...
// first make changes in parser.yy and scanner.ll files (use the corresponding files from level-1 of interpretation)
// then make the changes in ast.hh and ast.cc
// will have to make classes like Relational_Ast and so on
// most of the functions will be the same as in level-1 interpretor
// we will have to write for compile() and compile_and_optimize_ast() functions for these classes
