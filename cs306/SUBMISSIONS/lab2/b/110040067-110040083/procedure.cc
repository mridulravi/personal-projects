...
// We have to change this loop because in case of If Else conditions we have to go to blocks on basis of the value returned by the basic block
/*
	while (current_bb)
	{
...
// we can have only 4 types of statements, goto, if-else, assignment or RETURN statement
// so in all these cases we will change the value of the "value" field in Eval_field_Value_Int to get the next basic block number
// This will not work since here the return value is Eval_Result and that is Eval_Result_Value_Int
	...
	while (current_bb)
	{
		...
		// start from the beginning till you get the required 
			...
			{
				char msg[20];
				sprintf(msg, "bb %d doesn't exist", result->get_value());
