...
void Name_Ast::print_value(Local_Environment & eval_env, ostream & file_buffer)
{
	...
	{	// if type is int or float, then continue, otherwise throw an error
		...
		{
			file_buffer << std::setprecision(0)<<loc_var_val->get_value() << "\n";
	...
	{	// if type is int, then continue, otherwise throw an error
		if (glob_var_val->get_result_enum() == int_result)
		{
			if (glob_var_val == NULL)
				...
				file_buffer << std::setprecision(0) << glob_var_val->get_value() << "\n";
		}
...
//	cout<<"REaching here, op is "<<op<<endl;
	...
	if(lhs->get_data_type() == int_data_type)
	{
		...
		{
			case 0:
				result->set_value((int)(i.get_value() + j->get_value()));
				break;
			case 1:
				result->set_value((int)(i.get_value() - j->get_value()));
				break;
...
/*	switch(op)
	{
		case 0:
			result->set_value(i.get_value() + j->get_value());
			break;
		case 1:
			result->set_value(i.get_value() - j->get_value());
			break;
...
Eval_Result & GoTo_Ast::evaluate(Local_Environment & eval_env, ostream & file_buffer)
{
	...
	file_buffer << "\n" << AST_SPACE<<"GOTO (BB "<< std::setprecision(0) << result.get_value()<<")\n";
	return result;
}
...
Eval_Result & Conditional_Ast::evaluate(Local_Environment & eval_env, ostream & file_buffer)
{
	...
	if(temp.get_value() == 1)
	{
		...
		file_buffer << "\n" << AST_SPACE << "Condition True : Goto (BB "<<std::setprecision(0) << result->get_value()<<")\n";
	}
	else
	{
		result = &(falseSuccessor->evaluate(eval_env, file_buffer) );
		//file_buffer << "\n" << AST_SPACE << "Condition False : Goto (BB "<<result->get_value()<<")\n";
		file_buffer << "\n" << AST_SPACE << "Condition False : Goto (BB "<<std::setprecision(0) << result->get_value()<<")\n";
	}
