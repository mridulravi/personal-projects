...
int num_parent_pointer_redirections, num_iterations_for_algo, num_gvalue_update;
...
void parent_redirection(int index)
{
	...
	for(i=0;i<n.neighb_edge_ids.size();i++)
	{
		...
		else if(nodes[other_end].status==2)	//If the child is in CL then do recursively on its children
		{
			...
			{
				nodes[other_end].g_value=n.g_value+w;
				if(nodes[other_end].parent!=index)
...
int main(){
	...
	else 	// a path has been found
	{
		...
		cout<<"Number of iterations taken by the A* Algo ="<<num_iterations_for_algo<<endl;
		cout<<"Number of Parent Pointer Redirections ="<<num_parent_pointer_redirections<<endl;
		cout<<"Number of g value updates ="<<num_gvalue_update+num_parent_pointer_redirections<<endl; 
	}
}
